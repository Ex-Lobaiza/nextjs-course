...
> Capaz alguien mas experimentado en rutas `/[id]` dinámicas, se pregunte ¿Por qué no mostraba los datos actualizados? Bueno, porque la grilla de restaurantes en el índice tenía un componente `Link` por cada restaurante. El componente `Link` hace [`prefetching`](https://nextjs.org/docs/app/building-your-application/routing/linking-and-navigating#1-prefetching) por defecto, lo que en segundo plano generaba la ruta dinámica y la cacheaba de manera estática. Por eso tampoco veiamos un loading al entrar. Si queremos evitar esto, podemos agregar la prop `prefetch` en `false` al componente `Link` de nuestra grilla de restaurantes.
